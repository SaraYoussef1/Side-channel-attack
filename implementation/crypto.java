import java.math.BigInteger;
import java.util.Random;


class crypto {

    private static BigInteger encrypt(BigInteger m, BigInteger e, BigInteger modulus) {
        return modExp(m, e, modulus);
    }

    private static BigInteger decrypt(BigInteger c, BigInteger d, BigInteger modulus) {
        return modExp(c, d, modulus);
    }

    private static BigInteger modExp(BigInteger a, BigInteger exponent, BigInteger N) {

        // Note: This code assumes the most significant bit of the exponent is 1, i.e., the exponent is not zero.
        BigInteger result = a;
        int expBitlength = exponent.bitLength();
        for (int i = expBitlength - 2; i >= 0; i--) {
            result = result.multiply(result).mod(N);
            if (exponent.testBit(i)) {
                result = result.multiply(a).mod(N);
            }
        }
        return result;
    }

    public static void main(String[] args) {

        // RSA modulus
        BigInteger modulus = new BigInteger(
                "",
                16);

        // private exponent
        BigInteger d = new BigInteger(
                "",
                16);

        // public exponent (just provided for illustration. The focus in this assignment is on the decryption process)

        BigInteger e = new BigInteger("10001", 16);
        long startTime, endTime;
        Random rnd = new Random();
        BigInteger m, c, m2, cm, mm;

        montgomery mon = new montgomery(modulus);
        BigInteger RSAavr = BigInteger.ZERO, MONTavr = BigInteger.ZERO;

        //encrypt and decrypt 100 message using two methods; RSA and Montogomery to calculate average time.
        int sampleSize = 10;
        for(int i=0; i<sampleSize; i++){
            m = new BigInteger(modulus.bitLength() - 1, rnd);

            //using RSA

            c = encrypt(m, e, modulus);
            startTime = System.currentTimeMillis();
            m2 = decrypt(c, d, modulus);
            endTime = System.currentTimeMillis();
            RSAavr = RSAavr.add(BigInteger.valueOf(endTime-startTime));

            //using Montgomery
            cm = mon.encrypt(m, e);
            startTime = System.currentTimeMillis();
            mm = mon.decrypt(cm, d);
            endTime = System.currentTimeMillis();
            MONTavr = MONTavr.add(BigInteger.valueOf(endTime-startTime));

            if (!m.equals(m2))
                System.err.println("There is an error in RSA.");
            if (!mm.equals(m2))
                System.err.println("There is an error in Montogmery.");

        }
        System.out.println("RSA average is " + RSAavr.divide(BigInteger.valueOf(sampleSize)));
        System.out.println("Montgomery average is " + MONTavr.divide(BigInteger.valueOf(sampleSize)));
    }

}

